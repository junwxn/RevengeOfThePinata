...
f32 DistanceSqPointToSegment(AEVec2 const& point, AEVec2 const& segStart, AEVec2 const& segEnd)
{
    ...
    AEVec2Sub(&ab, &end, &start);
    AEVec2Sub(&ap, &p, &start);

    f32 abLenSq = AEVec2SquareLength(&ab);
    if (abLenSq <= 0.00001f)
        return AEVec2SquareDistance(&p, &start);

    f32 t = AEVec2DotProduct(&ap, &ab) / abLenSq;
    t = ClampFloat(t);

    AEVec2Scale(&scaledAB, &ab, t);
    AEVec2Add(&closest, &start, &scaledAB);

    return AEVec2SquareDistance(&p, &closest);
}
...
bool PointOnSegment(AEVec2 const& p, AEVec2 const& a, AEVec2 const& b)
{
    ...
    f32 cross = AEVec2CrossProductMag(&ab, &ap);
    if (fabsf(cross) > 0.0001f)
        return false;

    f32 dot = AEVec2DotProduct(&ap, &ab);
    if (dot < 0.0f)
        return false;
...
{
    f32 d0 = DistanceSqPointToSegment(a0, b0, b1);
    f32 d1 = DistanceSqPointToSegment(a1, b0, b1);
    f32 d2 = DistanceSqPointToSegment(b0, a0, a1);
    f32 d3 = DistanceSqPointToSegment(b1, a0, a1);
